...
profile, friends. It embeds the game core (`frontend/game/`) as a route/component
(e.g. `/play/:roomId`).
...
(`frontend/app/`) — e.g. `npm create vite@latest . -- --template react-ts` — and:
...
3. Build profile, friends list, online status, rankings, and match-history views against the
   REST endpoints described in `backend/README.md`.
